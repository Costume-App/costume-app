...
const PRODUCTION = "55555555-5555-4555-8555-555555555555";
...
test("POST combines each matched group in request order and returns counts plus the snapshot", async () => {
  ...
  expect(combinePerformers).toHaveBeenNthCalledWith(1, PRODUCTION, C, [D]);
  expect(combinePerformers).toHaveBeenNthCalledWith(2, PRODUCTION, A, [B]);
...
test("POST 404 when the production is not in the org", async () => {
  ...
  const res = await POST(req({ groups: [{ performerIds: [A, B], keepId: A }] }), ctx(PRODUCTION));
...
test("POST 400 on a malformed body", async () => {
  const res = await POST(req({ groups: [{ performerIds: [A] }] }), ctx(PRODUCTION));
  expect(res.status).toBe(400);
  expect(loadDuplicateGroups).not.toHaveBeenCalled();
});

test("POST 409 when a requested set is not a current group, and runs nothing", async () => {
  const res = await POST(req({ groups: [{ performerIds: [A, C], keepId: A }] }), ctx(PRODUCTION));
...
test("POST 409 when the requested keeper does not match the computed keeper, and runs nothing", async () => {
  const res = await POST(req({ groups: [{ performerIds: [A, B], keepId: B }] }), ctx(PRODUCTION));
  expect(res.status).toBe(409);
  await expect(res.json()).resolves.toEqual({ error: "The cast list changed. Reload and review again." });
  expect(combinePerformers).not.toHaveBeenCalled();
});
...
test("POST returns 404 for a non-UUID production id without touching data", async () => {
  const res = await POST(req({ groups: [{ performerIds: [A, B], keepId: A }] }), ctx("not-a-uuid"));
  expect(res.status).toBe(404);
